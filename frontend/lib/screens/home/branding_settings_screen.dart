...
class _BrandingSettingsScreenState extends State<BrandingSettingsScreen> {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error loading branding data: $e'),
  ...
  void _populateForm() {
    final brandingProvider = context.read<BrandingProvider>();
    final branding = brandingProvider.systemBranding;
    
    _appNameController.text = TypeConverter.safeToString(branding['app_name'] ?? '');
  ...
  Future<void> _uploadFile(String type) async {
    ...
    try {
      final result = kIsWeb 
          ? await brandingProvider.uploadSystemFileBytes(bytes!, type)
          : await brandingProvider.uploadSystemFile(file!, type);
  ...
  Future<void> _saveBranding() async {
    ...
    try {
      ...
      final success = await brandingProvider.updateSystemBranding(brandingData);
      
      if (success && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Branding settings saved successfully'),
            backgroundColor: Colors.green,
          ),
        );
      } else if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Failed to save branding settings'),
            backgroundColor: Colors.red,
          ),
        );
      }
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              const SizedBox(height: 8),
                              Text(
                                'Customize the appearance and branding of your retail management system',
                              ...
                              children: [
                                Expanded(
                                  child: _buildImageUpload(
                                    'logo',
                                    'App Logo',
                                    brandingProvider.getCurrentLogo(null),
                                  ),
                                ),
                                ...
                                Expanded(
                                  child: _buildImageUpload(
                                    'favicon',
                                    'Favicon',
                                    brandingProvider.getCurrentFavicon(null),
                                  ),
                                ),
                              ],
                            ),
                        ...
                        SizedBox(
                          ...
                          child: ElevatedButton.icon(
                            ...
                            label: Text(_isLoading ? 'Saving...' : 'Save Branding Settings'),
                            style: ElevatedButton.styleFrom(
