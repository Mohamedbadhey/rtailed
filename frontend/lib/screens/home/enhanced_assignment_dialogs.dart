...
class _ResetStoreDialogState extends State<ResetStoreDialog> {
          ...
          children: [
            ...
            Row(
              children: [
                ...
                const SizedBox(width: 8),
                Text(
                  t(context, 'Reset Store Business Assignments'),
                  style: Theme.of(context).textTheme.titleLarge?.copyWith(
                    fontWeight: FontWeight.bold,
                  ),
                ),
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      t(context, 'WARNING: This will remove ALL business assignments for the selected store. This action cannot be undone.'),
            ...
            if (_selectedStore != null && _currentAssignments.isNotEmpty) ...[
              Text(
                t(context, 'Current Business Assignments'),
                style: Theme.of(context).textTheme.titleMedium?.copyWith(
                  fontWeight: FontWeight.bold,
                ),
              ),
              ...
              Expanded(
                child: Container(
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      final business = _currentAssignments[index];
                      return ListTile(
            ...
            TextField(
              ...
              decoration: InputDecoration(
                labelText: t(context, 'Reason for Reset (Required)'),
                border: const OutlineInputBorder(),
                hintText: t(context, 'Explain why you are resetting this store...'),
              ),
              maxLines: 3,
            ),
            ...
            Row(
              ...
              children: [
                ...
                const SizedBox(width: 12),
                ElevatedButton(
                  onPressed: _isLoading || _selectedStore == null || _reasonController.text.trim().isEmpty
                      ? null
                      : _resetStoreAssignments,
                  style: ElevatedButton.styleFrom(
  ...
  Future<void> _loadCurrentAssignments() async {
    ...
    try {
      final assignments = await _apiService.getStoreBusinesses(_selectedStore!['id']);
      setState(() {
        _currentAssignments = assignments;
      });
    } catch (e) {
      print('Error loading current assignments: $e');
      setState(() {
        _currentAssignments = [];
      });
    }
  ...
  Future<void> _resetStoreAssignments() async {
    if (_selectedStore == null) return;
    
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: Text(t(context, 'Confirm Reset')),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(t(context, 'Are you absolutely sure you want to reset all business assignments for:')),
            const SizedBox(height: 8),
            Text(
              '${_selectedStore!['name']} (${_selectedStore!['store_code']})',
              style: const TextStyle(fontWeight: FontWeight.bold),
            ),
            const SizedBox(height: 8),
            Text('${_currentAssignments.length} assignments will be removed.'),
            const SizedBox(height: 8),
            Text(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            child: Text(t(context, 'Reset Store')),
          ),
        ],
      ),
    );
    ...
    if (confirmed == true) {
      ...
      try {
        final result = await _apiService.resetStoreBusinesses(
          _selectedStore!['id'],
        ...
        SuccessUtils.showBusinessSuccess(
          context,
          'Store reset successfully! ${result['removed_assignments']} assignments removed.',
        );
      } catch (e) {
